...
export class InventarioController {
  ...
  async findByBarcode(req: AuthRequest, res: Response) {
    try {
      return sendSuccess(res, await service.findByBarcode(req.params.codigo));
    } catch (e: unknown) { return sendError(res, e instanceof Error ? e.message : 'Error', 404); }
  }
  ...
  async update(req: AuthRequest, res: Response) {
    try {
      ...
      const prev   = await service.findById(id).catch(() => null);
      const dto    = updateArticuloSchema.parse(req.body);
      const saved  = await service.update(id, dto);
  ...
  async clone(req: AuthRequest, res: Response) {
    try {
      const saved = await service.clone(Number(req.params.id));
  ...
  async ajustarInventario(req: AuthRequest, res: Response) {
    try {
      ...
      const saved    = await service.ajustarInventario(id, cantidad, req.user?.id);
  ...
  async importarCSV(req: AuthRequest, res: Response) {
    try {
      ...
      const result = await service.importarCSV(csv, req.user?.id);
  ...
  async getMovimientos(req: AuthRequest, res: Response) {
    try {
      ...
      const { data, total, page, limit } = await service.getMovimientos(articuloId, req);
      return sendPaginated(res, data, total, page, limit);
    } catch (e: unknown) { return sendError(res, e instanceof Error ? e.message : 'Error'); }
  }
  ...
  async getStockBajo(req: AuthRequest, res: Response) {
    try {
      ...
      return sendSuccess(res, await service.getStockBajo(minimo));
    } catch (e: unknown) { return sendError(res, e instanceof Error ? e.message : 'Error'); }
  }

  async getCategorias(_req: AuthRequest, res: Response) {
    try { return sendSuccess(res, await service.getCategorias()); }
    catch (e: unknown) { return sendError(res, e instanceof Error ? e.message : 'Error'); }
  }
  ...
  async createCategoria(req: AuthRequest, res: Response) {
    try {
      ...
      return sendSuccess(res, await service.createCategoria(dto), 'Categoría creada', 201);
    } catch (e: unknown) { return sendError(res, e instanceof Error ? e.message : 'Error'); }
  }

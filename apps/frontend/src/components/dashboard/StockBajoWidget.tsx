      ...
      <div className="flex items-center justify-between px-5 py-4">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            {count > 0 && (
              <p className="text-xs text-amber-500 font-medium">{count} artículo{count !== 1 ? 's' : ''} crítico{count !== 1 ? 's' : ''}</p>
            )}
          </div>
        </div>
          ...
          <ul className="flex flex-col gap-0.5 px-2 pb-2">
            {data.map((art) => (
              <li key={art.id} className="flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-navy-50/80 transition-colors">
                <div className={`w-2 h-2 rounded-full shrink-0 ${art.cantidad === 0 ? 'bg-rose-500' : 'bg-amber-400'}`} />

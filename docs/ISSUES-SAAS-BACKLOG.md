...
- [x] **Aislamiento tenant — clientes, gastos, empleados, kits, proveedores:** `clientes-tenant-isolation.integration.test.ts` (GET ajeno → 404), `gastos-tenant-isolation.integration.test.ts` (GET ajeno → 403), `empleados-tenant-isolation.integration.test.ts` (GET ajeno → 404), `kits-tenant-isolation.integration.test.ts` y `proveedores-tenant-isolation.integration.test.ts` (GET ajeno → 403); ver `docs/arquitectura/TENANT-ISOLATION-INVENTORY.md`.

...
- [x] **Tests integración — aislamiento tenant:** `clientes-tenant-isolation.integration.test.ts` (GET ajeno → 404), `gastos-tenant-isolation.integration.test.ts` (GET ajeno → 403), `empleados-tenant-isolation.integration.test.ts` (GET ajeno → 404), `kits-tenant-isolation.integration.test.ts` y `proveedores-tenant-isolation.integration.test.ts` (GET ajeno → 403); inventario en `docs/arquitectura/TENANT-ISOLATION-INVENTORY.md` actualizado (2026-05-12).
